...
def create_relationship_helper(l_label, r_label, rel_entity):
    ...
    try:
        GraphRepo.execute_query(final_query, routing_control=Constants.WRITE)
    except Exception as e:
...
def check_relationship(relation, subject_label, predict_label):
    ...
    if not is_id_exists(relation[Constants.PREDICT], predict_label):
        return False
    if relation[Constants.FUNCTIONAL] == '1' and not is_functional_satisfies(relation[Constants.SUBJECT], subject_label, relation[Constants.NAME]):
        return False
    if relation[Constants.INVERSE_FUNCTIONAL] == '1' and not is_functional_satisfies(relation[Constants.PREDICT], predict_label, relation[Constants.NAME]):
        return False
    return True
